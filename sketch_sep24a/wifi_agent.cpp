...
void onReceive() {
  ...
  aiCam.sendDoc["A3"] = uxTaskGetStackHighWaterMark(RGB_Control.AID());
  aiCam.sendDoc["A4"] = uxTaskGetStackHighWaterMark(Car_Control.AID());
  ...
  // Sensor ultrasónico
  float us_result = int(us_distance * 100) / 100.0;
